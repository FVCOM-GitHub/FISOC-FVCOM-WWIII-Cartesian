...
101 msg = "OM failed to open stdoutFile "//OM_stdoutFile
  ...
  SUBROUTINE FISOC_OM_Wrapper_Init_Phase2(FISOC_config,vm,OM_ImpFB,OM_ExpFB,rc)
    ...
    LOGICAL   :: verbose_coupling, OM_initCavityFromWM, WM2OM_init_vars
    ...
    IF ((verbose_coupling).AND.(localPet.EQ.0)) THEN
       ...
       PRINT*,"******************************************************************************"
       PRINT*,""
       PRINT*,"Here we have access to the initialised WM fields, just in case the OM needs "
       PRINT*,"to know about these in order to complete its initialisation."
       PRINT*,""
    END IF

    CALL FISOC_ConfigDerivedAttribute(FISOC_config, WM2OM_init_vars, 'WM2OM_init_vars',rc=rc) 
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)

    IF (WM2OM_init_vars) THEN
  ...
  SUBROUTINE FISOC_OM_Wrapper_Run(FISOC_config,vm,OM_ExpFB,OM_ImpFB,rc_local)
    ...
    TYPE(ESMF_field)           :: WM_dTdz_l0,WM_z_l0, OM_bmb
    REAL(ESMF_KIND_R8),POINTER :: WM_dTdz_l0_ptr(:,:), WM_z_l0_ptr(:,:), OM_bmb_ptr(:,:)
    INTEGER                    :: OM_dt_sec
    ...
    IF (localPet.EQ.0) THEN
      ...
      WRITE (OM_outputUnit,*) 'FISOC is about to call FVCOM start time: ',trim(interval_startTime_char)
      WRITE (OM_outputUnit,*) 'FISOC is about to call FVCOM end   time: ',trim(interval_EndTime_char)
      IF ((verbose_coupling).AND.(localPet.EQ.0)) THEN
...
!    END IF
    ...
    IF ((verbose_coupling).AND.(localPet.EQ.0)) THEN
       ...
       IF ((PRESENT(OM_ExpFB)).AND.(.NOT.(PRESENT(OM_ImpFB)))) THEN
          PRINT*,"We have no new inputs for the OM from the WM.  We need to call the OM "
          PRINT*,"and record its output in the OM export field bundle."
       END IF

       IF ((PRESENT(OM_ExpFB)).AND.(PRESENT(OM_ImpFB))) THEN
          PRINT*,"We have new inputs for the OM from the WM in the OM import field bundle. "
       ...
       IF ((.NOT.(PRESENT(OM_ExpFB))).AND.(.NOT.(PRESENT(OM_ImpFB)))) THEN
          PRINT*,"We have no new inputs for the OM from the WM, and we do not need to "
...
!    INTEGER                               :: LBi, UBi, LBj, UBj ! tile start and end coords including halo
    ...
    fieldLoop: DO nn = 1,fieldCount
       ...
       CASE ('OM_UU')
         DO ii = 1,SIZE(ptr)
           ptr(ii) = UDOP(ownedNodeIds(ii))
         END DO
       CASE ('OM_VV')
         DO ii = 1,SIZE(ptr)
           ptr(ii) = VDOP(ownedNodeIds(ii))
         END DO
       CASE ('OM_ETA')
         DO ii = 1,SIZE(ptr)
           ptr(ii) = ELF(ownedNodeIds(ii))
         END DO
       CASE ('OM_HH')
         DO ii = 1,SIZE(ptr)
           ptr(ii) = H(ownedNodeIds(ii))
         END DO         
       CASE DEFAULT
...
!    PRINT*,'fieldCount is', fieldCount 
    ...
    fieldLoop: DO nn = 1,fieldCount
      ...
      IF (FISOC_WM2OM(fieldName,FISOC_config,rc=rc)) THEN
        ...
        CASE ('WM_HS')
          HSC1(1:size(ptr)) = ptr
	  WHERE(HSC1 < 0.0) HSC1 = 0.0
        CASE ('WM_DIR')
          DIRDEG1(1:size(ptr)) = ptr
        CASE ('WM_TPEAK')
          TPEAK(1:size(ptr)) = ptr
        CASE ('WM_TMBOT')
          Pwave_bot(1:size(ptr)) = ptr  
        CASE ('WM_UBOT')
          ub_swan(1:size(ptr)) = ptr 
        CASE ('WM_WLEN')
          WLEN(1:size(ptr)) = ptr   
        CASE ('WM_DIRBOT')
          DIRBOT(1:size(ptr)) = ptr   
        CASE ('WM_QB1')
          QB1(1:size(ptr)) = ptr   
        CASE DEFAULT
          msg = "ERROR: unknown variable: "//TRIM(ADJUSTL(fieldName))
  ...
  SUBROUTINE FVCOM2ESMF_mesh(FISOC_config,ESMF_FVCOMmesh,vm,rc)
    ...
    INTEGER                          :: localPet, petCount,numLocalNodes
...
!TODO: just use FVCOM vars directly, no need for intermediate vars, i.e. remove FVCOM_numNodes etc
    ...
    ! determine actual locally owned nodes
    ...
    DO ii = 1,SIZE(nodeOwners)
       ...
       END IF
    END DO
    deallocate(localNodeIDS)    

    allocate(localNodeIDS(numLocalNodes))
    numLocalNodes = 0
    DO ii = 1,SIZE(nodeOwnersGL_recv)
       IF (nodeOwnersGL_recv(ii).EQ.localPet) THEN
          numLocalNodes = numLocalNodes + 1
          localNodeIDS(numLocalNodes)=NLID(ii)
       END IF
    END DO
    ...
    DO ii = 1, FVCOM_numNodes
       ...
       CASE("spherical","Spherical","SPHERICAL")
          ...
          nodeCoords(nn+1) = LON(ii)
          nodeCoords(nn+2) = LAT(ii)
       CASE DEFAULT
    ...
    ! output mesh information from fvcom and esmf
    call ESMF_MeshGet(ESMF_FVCOMMesh,numOwnedNodes=mynp,numOwnedElements=myne,elementDistgrid=distgrid,rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    ...
    print*,'localPET=',localPET, 'fvcom owned nodes,elements',m,n
    print*,'localPET=',localPET, 'fvcom   all nodes,elements',mt,nt
    ...
    print*,'localPET=',localPET,"FINISHED FVCOM MESH CREATION"

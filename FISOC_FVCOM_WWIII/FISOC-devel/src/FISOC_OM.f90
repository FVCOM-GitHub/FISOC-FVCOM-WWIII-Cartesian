MODULE FISOC_OM_MOD
  ...
  PUBLIC FISOC_OM_setvm,FISOC_OM_register
...
CONTAINS
  ...
  !------------------------------------------------------------------------------
  SUBROUTINE FISOC_OM_setvm(comp, rc)
      
    type(ESMF_GridComp)     :: comp
    integer, intent(out)    :: rc

    CALL ESMF_LogWrite('OM_setvm', ESMF_LOGMSG_INFO)

    CALL ESMF_LogWrite('OM_setvm end', ESMF_LOGMSG_INFO)

    rc=ESMF_SUCCESS
  
  END SUBROUTINE FISOC_OM_setvm 
  ...
  ! initialisation of the OM and the second stage occurrs after the WM has been 
  ! initialised, to allow inter-component consistency checks or use of the WM 
...
# endif
  ...
  SUBROUTINE FISOC_OM_init_phase2(FISOC_OM, OM_ImpSt, OM_ExpSt, FISOC_clock, rc)
    ...
    msg = "OM initialise phase 2 (allows the OM access to the WM initial state) "
    CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
       line=__LINE__, file=__FILE__, rc=rc)
  ...
  SUBROUTINE FISOC_OM_run(FISOC_OM, OM_ImpSt, OM_ExpSt, FISOC_clock, rc)
    ...
    TYPE(ESMF_config)          :: FISOC_config
    TYPE(ESMF_Alarm)           :: alarm_OM_output, alarm_WM, alarm_WM_exportAvailable
    ...
    ! between the OM and WM.  Most of the logic is on the OM side since 
    ! it is assumed (actually it is required) that the WM timestep is 
    ...
    OM_output: IF (ESMF_AlarmIsRinging(alarm_OM_output, rc=rc)) THEN
       ...
       ! we have new WM output available and we need OM output
       WM_input1: IF (ESMF_AlarmIsRinging(alarm_WM_exportAvailable, rc=rc)) THEN          
       ...
       ! no new WM output is available.  we need OM output
       ELSE
    ...
    ELSE       
       ! we have new WM output available but we do not need OM output
       WM_input2: IF (ESMF_AlarmIsRinging(alarm_WM_exportAvailable, rc=rc)) THEN
       ...
       ! no new WM output is available and we do not need OM output
       ELSE
    ...
    OM_cumulate: IF (ESMF_AlarmIsRinging(alarm_OM_output)) THEN
       ...
       ! ...and process the cumulated outputs if this is an WM step
       IF (ESMF_AlarmIsRinging(alarm_WM)) THEN
    ...
    IF (ESMF_AlarmIsRinging(alarm_WM_exportAvailable, rc=rc)) THEN
       CALL ESMF_AlarmRingerOff(alarm_WM_exportAvailable, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)              
    END IF    
...
!    USE mod_param, ONLY       : BOUNDS, Ngrids
    ...
    TYPE(ESMF_Alarm)                         :: alarm_WM_exportAvailable
    INTEGER, SAVE                            :: linterpCounter=0
    INTEGER                                  :: dt_ratio, WM_dt_int

    REAL(ESMF_KIND_R8)          :: linterpFactor, WM_dt, OM_WCmin, CavCorr
    TYPE(ESMF_field)            :: WM_z_l0_previous, WM_z_l0_linterp 
    TYPE(ESMF_field)            :: WM_z_l0_field, OM_z_l0_field, dddt_field
    TYPE(ESMF_field)            :: OM_bed_field
    TYPE(ESMF_field)            :: OM_z_lts_field,WM_z_lts_field,WM_dsdt_field
# if defined(FISOC_OM_GRID)
    TYPE(ESMF_grid)             :: OM_grid
    REAL(ESMF_KIND_R8),POINTER  :: ptr_curr(:,:),ptr_prev(:,:),ptr_linterp(:,:)
    REAL(ESMF_KIND_R8),POINTER  :: WM_z_l0(:,:), OM_z_l0(:,:), dddt(:,:)
    REAL(ESMF_KIND_R8),POINTER  :: OM_bed(:,:)
    REAL(ESMF_KIND_R8),POINTER  :: OM_z_lts(:,:),WM_z_lts(:,:),WM_dsdt(:,:)
...
# endif
    ...
    CASE('CorrectedRate')
      ...
      ! WM var dddt will be used.  If an WM export is available, which means 
      ! dddt has just been calculated, we modify dddt here to impose a 
      ! correcting drift. The drift is designed to reduce the WM-OM 
      ! discrepancy over one WM timestep.
      CALL ESMF_ClockGetAlarm(FISOC_clock, "alarm_WM_exportAvailable", alarm_WM_exportAvailable, rc=rc)
      IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
           line=__LINE__, file=__FILE__)) &
           CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
      ...
      IF (ESMF_AlarmIsRinging(alarm_WM_exportAvailable, rc=rc)) THEN 
        ...
        ! ...and the WM cavity geom.
        CALL ESMF_FieldBundleGet(OM_ImpFB, fieldName="WM_z_l0", field=WM_z_l0_field, rc=rc)
        IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
             line=__LINE__, file=__FILE__)) &
             CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
        CALL ESMF_FieldGet(field=WM_z_l0_field, farrayPtr=WM_z_l0, rc=rc)
        IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
             line=__LINE__, file=__FILE__)) &
             CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
        ...
        ! drift correction.
        CALL ESMF_FieldBundleGet(OM_ImpFB, fieldName="WM_dddt", field=dddt_field, rc=rc)
        IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
             line=__LINE__, file=__FILE__)) &
             CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
        ...
        ! WM upper ice surface
        ! WM change rate of upper surface, dsdt
        UpperSurf: IF (CalculateUpperSurface) THEN          
          ...
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
               line=__LINE__, file=__FILE__)) &
               CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
          CALL ESMF_FieldBundleGet(OM_ImpFB, fieldName="WM_z_lts", field=WM_z_lts_field, rc=rc)
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
               line=__LINE__, file=__FILE__)) &
               CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
          CALL ESMF_FieldGet(field=WM_z_lts_field, farrayPtr=WM_z_lts, rc=rc)
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
               line=__LINE__, file=__FILE__)) &
               CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
          CALL ESMF_FieldBundleGet(OM_ImpFB, fieldName="WM_dsdt", field=WM_dsdt_field, rc=rc)
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
               line=__LINE__, file=__FILE__)) &
               CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
          CALL ESMF_FieldGet(field=WM_dsdt_field, farrayPtr=WM_dsdt, rc=rc)
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
               line=__LINE__, file=__FILE__)) &
               CALL ESMF_Finalize(endflag=ESMF_END_ABORT)          
        END IF UpperSurf
        
        ! WM time step in seconds is used (we are modifying dddt here in 
        ! metres per second)
        CALL FISOC_ConfigDerivedAttribute(FISOC_config, WM_dt_int, 'WM_dt_sec',rc)
        IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
             line=__LINE__, file=__FILE__)) &
             CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
        WM_dt = REAL(WM_dt_int,ESMF_KIND_R8)
        ...
        IF ( (SIZE(dddt).NE.SIZE(WM_z_l0)) .OR.     & 
        ...
        DO jj=computationalLBound(2),computationalUBound(2)
          DO ii=computationalLBound(1),computationalUBound(1)
            dddt(ii,jj) = dddt(ii,jj) +                          &
                 CavCorr*( MAX(WM_z_l0(ii,jj),OM_bed(ii,jj)+    &
                 OM_WCmin)-OM_z_l0(ii,jj) ) / WM_dt
            IF (CalculateUpperSurface) THEN          
              WM_dsdt(ii,jj) = WM_dsdt(ii,jj) +  CavCorr *     & 
                   (WM_z_lts(ii,jj)-OM_z_lts(ii,jj)) / WM_dt
            END IF
          END DO
        END DO
...
# endif
        ...
        NULLIFY(WM_z_l0,OM_z_l0,OM_bed,dddt,ptrX)
        IF (CalculateUpperSurface) THEN          
          NULLIFY(WM_dsdt,WM_z_lts,OM_z_lts)
        END IF
      END IF
 ...
 !      CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       ...
       ! If an WM export is available this means the WM cavity has been 
       ! updated.  Each time this happens we can reset a counter.  The 
       ! counter is used to count steps since the last WM cavity update. 
       ! We rely on dt_ratio governing the total number of steps until 
       ! the next WM cavity update. 
       ! Note: we assume the WM initialised both curr and previous cavity 
       ...
       CALL ESMF_FieldGet(field=WM_z_l0_field, farrayPtr=ptr_curr, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       
       CALL ESMF_FieldBundleGet(OM_ImpFB, fieldname='WM_z_l0_previous', field=WM_z_l0_previous, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       
       CALL ESMF_FieldGet(field=WM_z_l0_previous, farrayPtr=ptr_prev, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       
       CALL ESMF_FieldBundleGet(OM_ImpFB, fieldname='WM_z_l0_linterp', field=WM_z_l0_linterp, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)

       CALL ESMF_FieldGet(field=WM_z_l0_linterp, farrayPtr=ptr_linterp, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       ...
       ! now turn on WM export alarm, because the WM may not have turned 
       ! it on, but the time interpolated cavity is effectively a new WM 
       ! export, at least from the OM perspective
       CALL ESMF_AlarmRingerOn(alarm_WM_exportAvailable, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)              
       ...
       CASE("WM")
          IF (PRESENT(OM_ImpFB)) THEN
             IF (PRESENT(OM_ExpFB)) THEN
                ...
                CALL FISOC_FB2NC(OutputFileName,OM_ImpFB,FISOC_config)
          ELSE
                msg = "OM_NCfreq=WM but we have only OM_ImpFB and not OM_ExpFB... :("
                CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_ERROR, &
                     line=__LINE__, file=__FILE__, rc=rc)
                CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
             END IF

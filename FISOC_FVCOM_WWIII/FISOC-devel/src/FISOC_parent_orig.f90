    ...
    mainTimeStepping: DO WHILE (.NOT. ESMF_ClockIsStopTime(FISOC_clock, rc=rc))
       ...
       IF (ESMF_AlarmIsRinging(alarm_OM, rc=rc)) THEN
         ...
         IF ( WM_UseOMGrid .OR. OM_UseWMGrid ) THEN
           CALL FISOC_GridCompRun(FISOC_OM, WM_ExpSt, OM_ExpSt, profiling, &
         ...
         ELSE
           ...
           CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
                line=__LINE__, file=__FILE__, rc=rc)
           !jzge: I really do not know why StateReconcile(OM_ExpSt) fails
           !CALL ESMF_StateReconcile(OM_ExpSt, rc=rc)
           !IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
           !     line=__LINE__, file=__FILE__)) &
           !     CALL ESMF_Finalize(endflag=ESMF_END_ABORT)    
       ...
       IF (ESMF_AlarmIsRinging(alarm_WM, rc=rc)) THEN
         IF (verbose_coupling) THEN
           msg = "FISOC parent run: calling WM"
           CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
                line=__LINE__, file=__FILE__, rc=rc)
         END IF
         IF ( WM_UseOMGrid .OR. OM_UseWMGrid ) THEN
           CALL FISOC_GridCompRun(FISOC_WM, OM_ExpSt, WM_ExpSt, profiling, &
                vm, FISOC_clock=FISOC_clock, phase=1, cumulatedCPUtime=WM_time)
         ELSE
           CALL FISOC_GridCompRun(FISOC_WM, WM_ImpSt, WM_ExpSt, profiling, &
                vm, FISOC_clock=FISOC_clock, phase=1, cumulatedCPUtime=WM_time)
           
           msg = "FISOC parent reconciling states prior to regridding"
           CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
                line=__LINE__, file=__FILE__, rc=rc)
           !jzge: I really do not know why StateReconcile(WM_ExpSt) fails 
           !CALL ESMF_StateReconcile(WM_ExpSt, rc=rc)
           !IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
           !     line=__LINE__, file=__FILE__)) &
           !     CALL ESMF_Finalize(endflag=ESMF_END_ABORT)    
           CALL ESMF_StateReconcile(OM_ImpSt, rc=rc)
           IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, file=__FILE__)) &
                CALL ESMF_Finalize(endflag=ESMF_END_ABORT)    
         ...
         CALL ESMF_AlarmRingerOff(alarm_WM, rc=rc)
         IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=__LINE__, file=__FILE__)) &
              CALL ESMF_Finalize(endflag=ESMF_END_ABORT)              
       END IF

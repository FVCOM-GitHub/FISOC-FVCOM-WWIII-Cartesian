...
MODULE FISOC_utils_MOD
  ...
  PUBLIC   FISOC_getListFromConfig, FISOC_populateFieldBundle, &
       ...
       FISOC_destroyClocks, FISOC_WM2OM, FISOC_OM2WM,        &
...
CONTAINS
  ...
  LOGICAL FUNCTION FISOC_ActivePartitionCheck(rc_getAtt,XXXM_partitions,localpet, &
    ...
    ELSE
       ...
       CASE DEFAULT
          msg = "ComponentType needs to be OM or WM"
  ...
  !
  SUBROUTINE FISOC_MakePetList(VM, NumPartitions, PetList, ContextFlag, FirstPet, SecondPet)
    ...
    INTEGER,INTENT(IN),OPTIONAL         :: FirstPet,SecondPet

    INTEGER                             :: PetCount, NumPartitions_local
    INTEGER                             :: FirstPet_local, nn, rc

    IF (PRESENT(FirstPet).AND.(.NOT.PRESENT(SecondPet))) THEN
       FirstPet_local = FirstPet
    ELSEIF (PRESENT(FirstPet).AND.PRESENT(SecondPet)) THEN
       FirstPet_local = FirstPet + SecondPet
    ELSE
       FirstPet_local = 0
    END IF
  ...
  !
  SUBROUTINE FISOC_MPI_NewComm(MPI_COMM_in, SizeNew, MPI_COMM_new,WM_process)

    USE mpi

    INTEGER,INTENT(IN)  :: MPI_COMM_in, SizeNew
    INTEGER,INTENT(OUT) :: MPI_COMM_new
    LOGICAL,INTENT(OUT) :: WM_process
  ...
  LOGICAL FUNCTION FISOC_IsDerived_Real(FieldName,FISOC_config,rc)
    ...
    ListNameReq = 'FISOC_WM_ReqVars:'
    ListNameDer = 'FISOC_WM_DerVars:'
    ...
    ELSE
       msg= "FATAL: FieldName "//FieldName//" not found in WM_reqVars or WM_derVars."
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_ERROR, &
            line=__LINE__, file=__FILE__)
       CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  SUBROUTINE FISOC_cavityCheckOptions(FISOC_config,rc)
    ...
    CHARACTER(len=ESMF_MAXSTR),ALLOCATABLE:: WM2OM_Vars(:)
    ...
    label = 'WM2OM_vars:'
    CALL FISOC_getStringListFromConfig(FISOC_config, label, WM2OM_Vars,rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    count = 0
    DO ii=1,SIZE(WM2OM_Vars)
       
       IF (TRIM('WM_z_l0').EQ.TRIM(WM2OM_Vars(ii))) THEN
          IF (OM_cavityUpdate.NE.'RecentIce') cavityUpdateMismatch = .TRUE.
          count = count + 1
       END IF
       
       IF (TRIM('WM_z_l0_linterp').EQ.TRIM(WM2OM_Vars(ii))) THEN
          IF (OM_cavityUpdate.NE.'Linterp') cavityUpdateMismatch = .TRUE.
          count = count + 1
       END IF
       
       IF (TRIM('WM_dddt').EQ.TRIM(WM2OM_Vars(ii))) THEN
    ...
    IF (count.eq.0) THEN
       msg = 'no WM cavity variable will be passed to the OM'
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
            line=__LINE__, file=__FILE__, rc=rc)          
    END IF

    IF (count.gt.1) THEN
       msg = 'only 1 WM cavity variable should be passed to the OM'
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_ERROR, &
            line=__LINE__, file=__FILE__, rc=rc)
       CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    END IF
    ...
    IF (cavityUpdateMismatch) THEN
       msg = 'Cavity update mismatch (check WM2OM vars) for OM_cavityUpdate: '//OM_cavityUpdate
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_ERROR, &
            line=__LINE__, file=__FILE__, rc=rc)
       CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    END IF
  ...
  !------------------------------------------------------------------------------
  ! 
  ! check whether user wants this variable (fieldname) to be passed from the WM 
  ! to the OM.
  LOGICAL FUNCTION FISOC_WM2OM(fieldName,FISOC_config,rc)
    ...
    IF (rc.EQ.ESMF_RC_NOT_FOUND) THEN
       msg = "WM2OM_vars not found in FISOC config file, trying to pass all available vars"
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
            line=__LINE__, file=__FILE__, rc=rc)          
       FISOC_WM2OM = .TRUE. ! pass all vars if list is not present 
       RETURN
    ...
    FISOC_WM2OM = FISOC_listContains(fieldName,WM2OM_Vars,rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  LOGICAL FUNCTION FISOC_OM2WM(fieldName,FISOC_config,rc)

    CHARACTER(len=ESMF_MAXSTR),INTENT(IN) :: fieldName
    TYPE(ESMF_config),INTENT(INOUT)       :: FISOC_config
    INTEGER,INTENT(OUT),OPTIONAL          :: rc

    CHARACTER(len=ESMF_MAXSTR),ALLOCATABLE:: OM2WM_Vars(:)
    CHARACTER(len=ESMF_MAXSTR)            :: label
    INTEGER                               :: ii
    ...
    label = 'OM2WM_vars:'
    CALL FISOC_getStringListFromConfig(FISOC_config, label, OM2WM_Vars,rc=rc)
    IF (rc.EQ.ESMF_RC_NOT_FOUND) THEN
       msg = "OM2WM_vars not found in FISOC config file, trying to pass all available vars"
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
            line=__LINE__, file=__FILE__, rc=rc)          
       FISOC_OM2WM = .TRUE. ! pass all vars if list is not present 
       RETURN
    ELSE IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) THEN
       CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    ...
    FISOC_OM2WM = FISOC_listContains(fieldName,OM2WM_Vars,rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
...
!
!    CALL ESMF_AlarmDestroy(alarm_WM, rc=rc)
!    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
!         line=__LINE__, file=__FILE__)) &
!         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
...
!         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  SUBROUTINE FISOC_setClocks(FISOC_config, FISOC_clock, rc)
    ...
    INTEGER                 :: WM_dt_sec, OM_dt_sec, dt_ratio
    INTEGER                 :: start_year, end_year, start_month, end_month, start_day, end_day, start_hour, end_hour
    INTEGER                 :: OM_outputInterval, runLength_WM_steps, runLength_secs
    TYPE(ESMF_TimeInterval) :: WM_dt, OM_dt, runLength_timeInterval
    TYPE(ESMF_Time)         :: startTime, endTime
    TYPE(ESMF_Alarm)        :: alarm_OM, alarm_OM_output, alarm_WM, alarm_WM_exportAvailable
    LOGICAL                 :: gotRunLength
    character(len=ESMF_MAXSTR)       :: calendar
    ...
    CALL FISOC_ConfigDerivedAttribute(FISOC_config, WM_dt_sec, 'WM_dt_sec',rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__, rcToReturn=rc)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    ...
    CALL ESMF_ConfigGetAttribute(FISOC_config, start_day, label='start_day:', rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__, rcToReturn=rc)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    
    CALL ESMF_ConfigGetAttribute(FISOC_config, start_hour, label='start_hour:', rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__, rcToReturn=rc)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)

    CALL ESMF_ConfigGetAttribute(FISOC_config, runLength_WM_steps, label='runLength_WM_steps:', rc=rc)
    IF (rc.EQ.ESMF_RC_NOT_FOUND) THEN
       gotRunLength = .FALSE.
    ELSE IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    call ESMF_ConfigGetAttribute(FISOC_config, calendar, label='Calendar:', rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__, rcToReturn=rc)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    ...
    if (trim(calendar) == 'NO_LEAP') then
       ...
       msg = "Only NO_LEAP and GREGORIAN calendars currently supported:"//trim(calendar)
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_ERROR, &
            line=__LINE__, file=__FILE__, rc=rc)
       CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    end if
    ...
    IF (.NOT.gotRunLength) THEN
       ...
       CALL ESMF_ConfigGetAttribute(FISOC_config, end_day, label='end_day:', rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__, rcToReturn=rc)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)

       CALL ESMF_ConfigGetAttribute(FISOC_config, end_hour, label='end_hour:', rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__, rcToReturn=rc)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    END IF
    ...
    CALL ESMF_TimeSet(startTime, yy=start_year, mm=start_month, dd=start_day, h=start_hour, rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    
    IF (gotRunLength) THEN
       runLength_secs = runLength_WM_steps * WM_dt_sec
       CALL ESMF_TimeIntervalSet(runLength_timeInterval, s=runLength_secs, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
       endTime = startTime + runLength_timeInterval
       msg = "used runLength_WM_steps to set the end time"  
       CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
         line=__LINE__, file=__FILE__, rc=rc)       
    ELSE
       CALL ESMF_TimeSet(endTime, yy=end_year, mm=end_month, dd=end_day, h=end_hour, rc=rc)
       IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) &
            CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    END IF
    ...
    alarm_WM = ESMF_AlarmCreate(clock=FISOC_clock, name="alarm_WM", &
         ringTime=((startTime+WM_dt)-OM_dt), ringInterval=WM_dt, sticky=.FALSE., rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
    
    alarm_WM_exportAvailable = ESMF_AlarmCreate(clock=FISOC_clock, &
         name="alarm_WM_exportAvailable", &
         ringTime=(startTime+WM_dt), ringInterval=WM_dt, sticky=.FALSE.,rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
...
# endif
  ...
  SUBROUTINE FISOC_ConfigDerivedAttributeRegridMethod(FISOC_config, derivedAttribute, label,rc)
    ...
    CASE('WM2OM_regrid:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, regridMethodChar, label='WM2OM_regrid:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          regridMethodChar = "ESMF_REGRIDMETHOD_BILINEAR"
          msg = "WARNING: WM2OM_regrid not found, setting to bilinear."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
    ...
    CASE('OM2WM_regrid:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, regridMethodChar, label='OM2WM_regrid:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          regridMethodChar = "ESMF_REGRIDMETHOD_BILINEAR"
          msg = "WARNING: OM2WM_regrid not found, setting to bilinear."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
...
!       derivedAttribute = ESMF_REGRIDMETHOD_CONSERVE_2ND
  ...
  SUBROUTINE FISOC_ConfigDerivedAttributeExtrapMethod(FISOC_config, derivedAttribute, label,rc)
    ...
    CASE('WM2OM_extrap:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, extrapMethodChar, label='WM2OM_extrap:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          extrapMethodChar = "ESMF_EXTRAPMETHOD_NONE"
          msg = "WARNING: WM2OM_extrap not found, setting to ESMF_EXTRAPMETHOD_NONE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    CASE('OM2WM_extrap:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, extrapMethodChar, label='OM2WM_extrap:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          extrapMethodChar = "ESMF_EXTRAPMETHOD_NONE"
          msg = "WARNING: OM2WM_extrap not found, setting to ESMF_EXTRAPMETHOD_NONE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
  ...
  SUBROUTINE FISOC_ConfigDerivedAttributeLogical(FISOC_config, derivedAttribute, label,rc)
    ...
    CASE('WM2OM_init_vars','WM2OM_init_vars:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='WM2OM_init_vars:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = .TRUE.
          msg = "WARNING: WM2OM_init_vars not found, setting to .TRUE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    CASE('OM2WM_init_vars','OM2WM_init_vars:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='OM2WM_init_vars:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = .TRUE.
          msg = "WARNING: OM2WM_init_vars not found, setting to .TRUE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    CASE('WM_maskOMvars','WM_maskOMvars:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='WM_maskOMvars:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = .FALSE.
          msg = "WARNING: WM_maskOMvars not found, setting to .FALSE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    CASE('WM_UseOMGrid','WM_UseOMGrid:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='WM_UseOMGrid:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = .FALSE.
          msg = "WM_UseOMGrid not found, setting to .FALSE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
    ...
    CASE('OM_UseWMGrid','OM_UseWMGrid:')
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='OM_UseWMGrid:', rc=rc_local)
       IF (rc_local.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = .FALSE.
          msg = "OM_UseWMGrid not found, setting to .FALSE."
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_INFO, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc_local, msg=ESMF_LOGERR_PASSTHRU, &
  ...
  SUBROUTINE FISOC_ConfigDerivedAttributeString(FISOC_config, derivedAttribute, label,rc)
   ...
   CASE("WM_coords")
       CALL ESMF_ConfigGetAttribute(FISOC_config, derivedAttribute, label='WM_coords:', rc=rc)
       IF (rc.EQ.ESMF_RC_NOT_FOUND) THEN
          derivedAttribute = "Cartesian"
          msg = "WM_coords not found, setting to Cartesian"
          CALL ESMF_LogWrite(msg, logmsgFlag=ESMF_LOGMSG_WARNING, &
               line=__LINE__, file=__FILE__)
       ELSE
          IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
  ...
  SUBROUTINE FISOC_locallyOwnedNodes(localPet,localNodeIDS,nodeOwners,ownedNodeIDs)
    ...
    LON_count = 0
    DO ii = 1,SIZE(nodeOwners)
       IF (nodeOwners(ii).EQ.localPet) THEN
          LON_count = LON_count + 1
       END IF
    END DO
    !TODO: fail fast if already allocated/associated
    ALLOCATE(ownedNodeIDs(LON_count)); ownedNodeIDs = 0

    LON_count = 0
    DO ii = 1,SIZE(nodeOwners)
       IF (nodeOwners(ii).EQ.localPet) THEN
          LON_count = LON_count + 1
          ownedNodeIDs(LON_count) = localNodeIDs(LON_count)
       END IF
    END DO
